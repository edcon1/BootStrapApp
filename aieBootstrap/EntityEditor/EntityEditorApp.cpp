...
bool EntityEditorApp::startup() {
	...
	HANDLE numEntitiesHandle = OpenFileMapping(FILE_MAP_ALL_ACCESS, false, L"EnitiyCountMemory");
	assert(numEntitiesHandle != nullptr);

	int* numEnitiesPtr = (int*)MapViewOfFile(
		numEntitiesHandle,
		FILE_MAP_ALL_ACCESS,
		0, 0, sizeof(int));

	m_entities.resize(*numEnitiesPtr);

	UnmapViewOfFile(numEnitiesPtr);
	CloseHandle(numEntitiesHandle);
